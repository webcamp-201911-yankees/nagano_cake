class Admin::OrderHistoriesController < ApplicationController
  ...
  private
  def order_params
	params.require(:order_history).permit(:payment_method, :order_status, :customer_id, :shipping_zipcode, :shipping_address, :name, :shipping_fee, :total_price, order_details_attributes:[:id,:prepare_status])
  end
end
